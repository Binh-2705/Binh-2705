...
start "HRM Queue Worker" cmd /k "cd /d "%~dp0laravel_app" && php artisan queue:work --queue=payroll,default --sleep=3 --tries=2 --timeout=310 --max-time=3600"
...
start "HRM Chatbot Service" cmd /k "cd /d "%~dp0bot_service" && python -m uvicorn app:app --host 127.0.0.1 --port 8001 --reload"
