...
class ReportController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('baocao.edit', ['report' => $reportId])
                ->with('success', 'Da tao bao cao thanh cong.');
        } catch (QueryException $exception) {
            return back()->withInput()->withErrors(['form' => 'Khong the tao bao cao: ' . $exception->getMessage()]);
        }
    }
    ...
    public function update(Request $request, int $report): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('baocao.edit', ['report' => $report])
                ->with('success', 'Da cap nhat bao cao thanh cong.');
        } catch (QueryException $exception) {
            return back()->withInput()->withErrors(['form' => 'Khong the cap nhat bao cao: ' . $exception->getMessage()]);
        }
    }
    ...
    public function destroy(int $report): RedirectResponse
    {
        try {
            ...
            return redirect()->route('baocao.index')
                ->with('success', 'Da xoa bao cao thanh cong.');
        } catch (QueryException $exception) {
            return back()->withErrors(['form' => 'Khong the xoa bao cao.']);
        }
    }
    ...
    {
        $rows = $this->reportService->exportRows($request->only(['q', 'type']))->values();
