...
class ChatbotBizController extends Controller
{
    ...
    public function show(int $id): JsonResponse
    {
        ...
        if (!$session) {
            return response()->json(['ok' => false, 'message' => 'Session khong ton tai.'], 404);
        }
    ...
    public function upsertSession(Request $request): JsonResponse
    {
        ...
        $username   = trim((string) $request->input('username', 'unknown'));
        $roleName   = trim((string) $request->input('role_name', 'unknown'));

        if ($sessionKey === '') {
            return response()->json(['ok' => false, 'message' => 'session_key is required.'], 422);
        }
    ...
    public function logMessage(Request $request): JsonResponse
    {
        ...
        $role              = trim((string) $request->input('role_name', ''));
        $content           = (string) $request->input('content', '');
        $source            = trim((string) $request->input('source_name', ''));
        $actions           = $request->input('actions', []);
        $suggestions       = $request->input('suggestions', []);
        $actionDraftToken  = $request->input('action_draft_token');

        if ($sessionId <= 0 || $role === '') {
            return response()->json(['ok' => false, 'message' => 'session_id and role_name are required.'], 422);
        }
    ...
    {
        $sessionId  = (int) $request->input('session_id', 0);
        $createdBy  = (int) $request->input('created_by', 0);
        $draft      = $request->input('draft', []);

        if ($sessionId <= 0 || $createdBy <= 0 || empty($draft)) {
            return response()->json(['ok' => false, 'message' => 'session_id, created_by and draft are required.'], 422);
        }
    ...
    public function getPendingDraft(Request $request): JsonResponse
    {
        $token     = trim((string) $request->input('token', ''));
        $accountId = (int) $request->input('account_id', 0);

        if ($token === '' || $accountId <= 0) {
            return response()->json(['ok' => false, 'message' => 'token and account_id are required.'], 422);
        }
    ...
    public function updateDraftStatus(Request $request, int $id): JsonResponse
    {
        $data = array_filter([
    ...
    public function executeDraft(Request $request): JsonResponse
    {
        ...
        if ($actionType === 'leave_approve') {
            ...
            try {
                $result = $connection->transaction(function () use ($connection, $leaveId) {
                    ...
                    return ['ok' => true, 'message' => 'Đã duyệt đơn nghỉ phép từ chatbot.', 'state_diff' => 'Trạng thái: ' . $status . ' -> Đã duyệt'];
                });
                return is_array($result) ? response()->json($result, $result['ok'] ? 200 : 422) : response()->json(['ok' => false, 'message' => 'Không thể duyệt.'], 500);
            } catch (\Throwable) {
        ...
        if ($actionType === 'leave_reject') {
            ...
            return response()->json(['ok' => true, 'message' => 'Đã từ chối đơn nghỉ phép từ chatbot.', 'state_diff' => 'Trạng thái: ' . $status . ' -> Từ chối']);
        }
