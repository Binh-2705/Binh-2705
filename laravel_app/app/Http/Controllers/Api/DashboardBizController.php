...
class DashboardBizController extends Controller
{
    ...
    public function markNotificationsRead(Request $request): JsonResponse
    {
        ...
        $leaveCount    = DB::connection($hrConn)->table('nghiphep')->where('TrangThai', 'Chờ duyệt')->count();
        $contractCount = DB::connection($hrConn)->table('hopdong')->whereNotNull('NgayKetThuc')->where('NgayKetThuc', '<=', now()->addDays(30)->toDateString())->count();
        $candidateCount = DB::connection($recruitConn)->table('hosoungtuyen')->where('NgayNop', '>=', now()->subDays(7)->toDateString())->count();
    ...
    private function recentRecruitment(int $limit): array
    {
        return DB::connection($this->conn('recruitment'))->table('dottuyendung')
            ...
            ->map(fn ($i) => [
                'type' => 'Tuyen dung', 'permission' => 'xem_dot_tuyen',
    ...
    private function recentTraining(int $limit): array
    {
        return DB::connection($this->conn('training'))->table('khoadaotao')
            ...
            ->orderByDesc('TuNgay')->limit($limit)->get()
            ->map(fn ($i) => [
                'type' => 'Dao tao', 'permission' => 'xem_khoa_dao_tao',
                'title' => $i->TenKhoaDaoTao,
                'description' => trim(($i->DonViToChuc ?: 'Noi bo') . ' • ' . $i->TrangThai),
                'at' => (string) $i->TuNgay, 'sort_at' => (string) $i->TuNgay,
    ...
    private function recentReports(int $limit): array
    {
        return DB::connection($this->conn('reporting'))->table('baocao')
            ...
            ->map(fn ($i) => [
                'type' => 'Bao cao', 'permission' => 'xem_baocao',
                'title' => $i->TenBaoCao,
