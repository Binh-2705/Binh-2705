...
@php $subtitle = 'Giám sát tin nhắn và action draft của chatbot' @endphp
@extends('layouts.app')
...
<section class="panel">
    <h3 class="no-top-margin">Action drafts</h3>
    <div class="table-shell">
        <table class="data-table table-compact">
            <thead><tr><th>Token</th><th>Action</th><th>Trạng thái</th><th>Tạo lúc</th></tr></thead>
            <tbody>
                @forelse ($drafts as $draft)
                    <tr><td>{{ $draft->token }}</td><td>{{ $draft->title }}</td><td>{{ $draft->status_name }}</td><td>{{ $draft->created_at }}</td></tr>
                @empty
                    <tr><td colspan="4" class="muted">Không có draft.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
...
<section class="panel">
    <h3 class="no-top-margin">Tin nhan</h3>
