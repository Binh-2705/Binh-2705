...
@section('content')
    <div class="header-top">
        <div>
            ...
            <p class="muted top-gap-md">Tài khoản: <strong>{{ $taiKhoan['TenDangNhap'] ?? 'N/A' }}</strong></p>
        </div>
    ...
    (function () {
        ...
        var url = (document.getElementById('dashboardChartData') || {}).getAttribute
            ? document.getElementById('dashboardChartData').getAttribute('data-charts-url')
            : null;
        if (!url) return;

        var CACHE_KEY = 'hrm_dash_charts';
        var CACHE_TTL = 300000; // 5 phút (ms)
        var cacheNote = document.getElementById('chartCacheNote');
        var refreshBtn = document.getElementById('btnRefreshCharts');

        function renderCharts(c) {
            // Phòng ban
            if (c.department && c.department.labels.length) {
            ...
            // Tuyển dụng / lương
            if (c.recruitment && c.recruitment.labels.length) {
                makeDoughnut(showChart('chartBox-recruitment','chartRecruitment'), c.recruitment.labels, c.recruitment.values);
            } else if (c.payroll && c.payroll.labels.length) {
                document.querySelector('#chartBox-recruitment h3').textContent = '💰 Lương theo trạng thái';
        ...
        function loadCharts(forceRefresh) {
            // Thử đọc sessionStorage trước
            if (!forceRefresh) {
                try {
            ...
            var fetchUrl = forceRefresh ? url + '?refresh=1' : url;
