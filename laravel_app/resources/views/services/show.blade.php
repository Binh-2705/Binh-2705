...
@php $subtitle = 'Chi tiết tài nguyên và dữ liệu đang được map qua cổng kết nối dịch vụ' @endphp
@extends('layouts.app')
...
@section('content')
    <section class="panel">
        <div class="toolbar">
            <div>
                ...
                <div class="page-note top-gap-md">API: <span class="code-chip">GET /api/services/{{ $service }}/{{ $resource }}</span></div>
    ...
    <section class="panel">
        @if (session('success'))
            <div class="flash-alert flash-success flash-inline">{{ session('success') }}</div>
        @endif
        ...
        @if ($items->isEmpty())
            <div class="service-empty top-gap-lg">Không có dữ liệu.</div>
        @else
