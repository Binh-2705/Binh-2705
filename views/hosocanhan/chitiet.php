...
<div class="container">
    ...
    <main class="main-content">
        ...
        <div class="profile-header-box">
            ...
            <div>
                ...
                <p style="margin:5px 0 0 0; opacity:0.9;">
                    Mã NV: <?= $nv['MaNV'] ?> | <?= $nv['TenChucVu'] ?? 'Nhân viên' ?>
                </p>
            </div>
        </div>
        ...
        <div class="profile-info">
            ...
            <div class="info-item"><label>Ngày sinh</label>
                <span><?= date('d/m/Y', strtotime($nv['NgaySinh'])) ?></span>
            </div>
            <div class="info-item"><label>Phòng ban</label>
                <span><?= $nv['TenPB'] ?? 'Chưa phân công' ?></span>
            </div>
            <div class="info-item"><label>Chức vụ</label>
                <span><?= $nv['TenChucVu'] ?? 'Chưa cập nhật' ?></span>
            </div>
            <div class="info-item"><label>Lương tháng gần nhất</label>
                <span>
                <?= ($luong) ? number_format($luong['TongLuong']).' VNĐ' : 'Chưa có dữ liệu' ?>
                </span>
            </div>
        </div>
        ...
        <!-- HỢP ĐỒNG -->
        <?php if ($hopdong): ?>
        <h2 style="margin-top:30px;">📄 Hợp đồng hiện tại</h2>
        <div class="profile-info">
            <div class="info-item"><label>Loại hợp đồng</label><span><?= $hopdong['LoaiHopDong'] ?></span></div>
            <div class="info-item"><label>Ngày bắt đầu</label>
                <span><?= date('d/m/Y', strtotime($hopdong['NgayBatDau'])) ?></span>
            </div>
            <div class="info-item"><label>Ngày kết thúc</label>
                <span><?= $hopdong['NgayKetThuc'] 
                    ? date('d/m/Y', strtotime($hopdong['NgayKetThuc'])) 
                    : 'Không thời hạn' ?></span>
            </div>
            <div class="info-item"><label>Trạng thái</label><span><?= $hopdong['TrangThai'] ?></span></div>
        </div>
        <?php endif; ?>

        <!-- CHẤM CÔNG -->
        <?php if ($chamcong): ?>
        <h2 style="margin-top:30px;">🕒 Chấm công tháng <?= $chamcong['Thang'] ?></h2>
        <div class="profile-info">
            <div class="info-item"><label>Số ngày làm</label><span><?= $chamcong['SoNgayLam'] ?></span></div>
            <div class="info-item"><label>Số ngày nghỉ</label><span><?= $chamcong['SoNgayNghi'] ?></span></div>
            <div class="info-item"><label>Ghi chú</label><span><?= $chamcong['GhiChu'] ?? '—' ?></span></div>
        </div>
        <?php endif; ?>

        <!-- KHEN THƯỞNG / KỶ LUẬT -->
        <h2 style="margin-top:30px;">🏅 Khen thưởng - Kỷ luật</h2>
        <?php if ($ktkl && mysqli_num_rows($ktkl) > 0): ?>
        <table class="table">
            <thead>
                <tr>
                    <th>Loại</th>
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($ktkl)): ?>
                <tr>
                    <td><?= $row['LoaiQD'] ?></td>
                    <td><?= $row['TieuDe'] ?></td>
                    <td><?= date('d/m/Y', strtotime($row['NgayRaQD'])) ?></td>
                    <td><?= number_format($row['GiaTri']) ?></td>
                </tr>
                <?php endwhile; ?>
            </tbody>
        </table>
        <?php else: ?>
            <p style="color:#64748b;">Chưa có khen thưởng hoặc kỷ luật</p>
        <?php endif; ?>
